use std::{iter::InPlaceIterable, net::{IpAddr, Ipv4Addr, SocketAddr, ToSocketAddrs, UdpSocket}, os::unix::net::SocketAddr};
...
impl Repeater {
        ...
        while let Ok((amt, src)) = self.socket.recv_from(&mut buf) {
            ...
            match header.command.into() {
                ...
                Command::CA_REPEATER_REGISTER => {
                    ...
                    let mut confirm_buf = MessageHeader {
                    ...
                    client_socket.send_to(confirm_buf, src);
    ...
    pub fn send_confirm(addr: SocketAddr) {
